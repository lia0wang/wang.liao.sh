...
import type { FrontMatter, Post, rehypeRawFrontMatter } from '~/types';
...
export async function getAllPostsFrontMatter(): Promise<Array<FrontMatter>> {
	...
	return files
		.map((slug) => {
			...
			const frontmatter = data as rehypeRawFrontMatter;
...
export async function getPost(slug: string): Promise<Post> {
	const rehypeRaw = readFileSync(join(BLOG_POSTS_DIR, `${slug}.md`)).toString();
	const { content, data } = matter(rehypeRaw);
